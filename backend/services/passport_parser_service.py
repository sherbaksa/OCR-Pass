...
class PassportParserService:
    ...
    def parse_passport_text(self, ocr_text: str) -> Dict[str, any]:
        """
        ...
        # Извлечение адреса регистрации
        address, conf_addr = self._extract_registration_address(normalized_text)
        if address:
            result['fields']['registration_address'] = address
            result['confidences']['registration_address'] = conf_addr
    ...
    def _extract_registration_address(self, text: str) -> Tuple[Optional[str], float]:
        """Извлечение адреса регистрации"""
